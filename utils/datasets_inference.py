...
class LoadClipsAndLabels(Dataset):
    ...
    def get_video_length(self, index):
        ...
        return int(self.video_length_dict[f"{video_id}"])
    
    def sample_temporal_frames(self, index):
        n_frames = self.get_video_length(index)
        current_frame_id = int(os.path.basename(self.img_files[index]).split(".")[0].split("_")[-1])
        ...
        sample_frame_paths = [
            ...
            for sample_frame_id in sample_frame_ids
                for ext in ['.png', '.jpg']
                    if (path := Path.joinpath(image_file_parent_path, f"Clip_{clip_id}_{str(sample_frame_id).zfill(5)}{ext}")).exists()
        ]
